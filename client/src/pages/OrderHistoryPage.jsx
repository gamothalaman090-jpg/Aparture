              ...
              >
                <div>
                  ...
                  <div className="flex items-center space-x-4 my-4">
                    <img
                      src={b.cameraId?.imageUrl || b.cameraId?.imageUrls?.[0] || b.camera?.imageUrl || b.camera?.imageUrls?.[0] || '/images/cinema_rig_onset.jpg'}
                      alt={b.cameraId?.name || b.camera?.name}
                      className="w-16 h-16 rounded-xl object-cover border border-white/10 shrink-0"
                    />
                    <div>
                      <span className="text-[10px] font-mono text-cyan-400 uppercase font-bold">
                        {b.cameraId?.brand || b.camera?.brand || 'Aperture'}
                      </span>
                      <h3 className="text-base font-bold text-white font-display line-clamp-1">
                        {b.cameraId?.name || b.camera?.name || 'Cinema Gear'}
                      </h3>
                      <div className="text-xs font-mono text-slate-400 mt-1">
                        Duration: <span className="text-amber-400 font-bold">{b.totalDays || 1} Days</span> &bull; Rate: {formatCurrency(b.dailyRateSnapshot || b.cameraId?.dailyRate || b.camera?.dailyRate)}/day
                      </div>
                    </div>
                  </div>
